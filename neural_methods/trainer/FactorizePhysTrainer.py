    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                labels = np.squeeze(batch[1][:,0:1,:]).to(self.device)
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                data, labels = valid_batch[0].to(self.device), np.squeeze(valid_batch[1][:,0:1,:]).to(self.device)
                if len(labels.shape) > 2:
                    labels = labels[..., 0]     # Compatibility wigth multi-signal labelled data
                labels = (labels - torch.mean(labels)) / torch.std(labels)  # normalize
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                data, labels_test = test_batch[0].to(self.device), np.squeeze(test_batch[1][:,0:1,:]).to(self.device)
