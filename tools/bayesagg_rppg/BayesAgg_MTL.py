...
class LastLayerPosteriorRegression(LastLayerPosterior):
    ...
    def compute_posterior(self, last_layer_params, features, labels,
        ...
        # 1) If full data is provided once, store its posterior for later
        if full_train_features is not None:
            prior_m, prior_P = self.prior(full_train_features, output_dim)
            self.full_data_posterior = self.posterior(full_train_features,
                                                      full_train_labels,
                                                      prior_m, prior_P)

        # 2) Otherwise fall back to simple Gaussian prior
        if self.full_data_posterior is None:
            prior_mean, prior_precision = self.prior(features, output_dim)
        else:
            prior_mean      = self.full_data_posterior.mean
            prior_precision = self.full_data_posterior.precision_matrix[0]

        # ---- update with (mini‑)batch ----------------------------------
        return self.posterior(features, labels, prior_mean, prior_precision)
